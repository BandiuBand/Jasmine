"""CLI debug tool for inspecting Jasmine v2 memory snapshot draft.

Usage:
    python -m jasmine_v2.memory.inspect_snapshot_draft \
...
def format_draft_text(draft_text: str | None, max_chars: int) -> str:
    """Format draft text, truncating if necessary."""
    if draft_text is None:
        return "(no draft_text)"
    if len(draft_text) <= max_chars:
        return draft_text
    return draft_text[:max_chars] + f"... [truncated, total chars: {len(draft_text)}]"


def inspect_snapshot_draft(
    group_id: str | None = None,
    space_str: str | None = None,
    date_str: str | None = None,
    ...
    """Inspect snapshot draft and return formatted output string."""
    ...
    lines = [
        ...
        "",
        "draft_text:",
        format_draft_text(draft_text, max_chars),
    ]
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point."""
    parser = argparse.ArgumentParser(description="Inspect Jasmine v2 memory snapshot draft")
    parser.add_argument(
        "--group-id",
        default=None,
    ...
    parser.add_argument(
        "--no-raw-episodes",
        action="store_true",
        help="Exclude raw episode content from snapshot.",
    )
    parser.add_argument(
        "--mode",
        default="raw",
        help="Snapshot mode. Only 'raw' is supported. Default is raw.",
    )
    parser.add_argument(
        "--max-chars",
        type=int,
        default=4000,
        help="Maximum characters of draft_text to display. Default is 4000.",
    )
    ...
    if args.mode != "raw":
        print("Error: mode must be 'raw'", file=sys.stderr)
        return 2
