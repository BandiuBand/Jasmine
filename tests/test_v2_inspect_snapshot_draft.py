"""Tests for jasmine_v2.memory.inspect_snapshot_draft module.

Run directly with: python3 tests/test_v2_inspect_snapshot_draft.py
"""
...
from jasmine_v2.memory.inspect_snapshot_draft import (
    format_draft_text,
    inspect_snapshot_draft,
    main,
    resolve_group_id,
)
...
def test_output_includes_counts():
    """Test inspect_snapshot_draft output includes counts."""
    fake_client = Mock()
    fake_client.snapshot_draft.return_value = {
...
def test_output_includes_draft_text():
    """Test inspect_snapshot_draft output includes draft_text."""
    fake_client = Mock()
    fake_client.snapshot_draft.return_value = {
        "group_id": "jv2-chat-test-day-2026-05-11",
        "mode": "raw",
        "counts": {"episodes": 1, "entities": 2, "edges": 3, "mentions": 4},
        "source": {"episodes": 1, "entities": 2, "edges": 3, "mentions": 4},
        "draft_text": "This is the snapshot draft text.",
    }
    ...
    assert "draft_text:" in output
    assert "This is the snapshot draft text." in output
    print("  output includes draft_text")
...
def test_no_raw_episodes_passes_false():
    ...
    call_kwargs = fake_client.snapshot_draft.call_args[1]
...
def test_max_chars_truncates_long_draft():
    """Test --max-chars truncates long draft_text."""
    fake_client = Mock()
    fake_client.snapshot_draft.return_value = {
        "group_id": "jv2-chat-test-day-2026-05-11",
        "mode": "raw",
    ...
    assert "... [truncated, total chars: 5000]" in output
    assert "A" * 101 not in output
    print("  max-chars truncates long draft")


def test_format_draft_text_none():
    """Test format_draft_text handles None."""
    result = format_draft_text(None, 100)
    assert result == "(no draft_text)"
    print("  format_draft_text handles None")


def test_format_draft_text_short():
    """Test format_draft_text does not truncate short text."""
    result = format_draft_text("short", 100)
    assert result == "short"
    print("  format_draft_text short text unchanged")
...
def test_main_with_group_id():
    ...
    fake_client.snapshot_draft.return_value = {
        "group_id": "jv2-chat-test-day-2026-05-11",
        "mode": "raw",
        "counts": {"episodes": 0, "entities": 0, "edges": 0, "mentions": 0},
        "source": {"episodes": 0, "entities": 0, "edges": 0, "mentions": 0},
        "draft_text": "",
    }

    import jasmine_v2.memory.inspect_snapshot_draft as inspect_module
...
def test_main_limits_passed():
    """Test main passes custom limits to snapshot_draft."""
    fake_client = Mock()
    fake_client.snapshot_draft.return_value = {
        "group_id": "jv2-chat-test-day-2026-05-11",
        "mode": "raw",
...
def run_all_tests():
    ...
    tests = [
        ...
        test_output_includes_draft_text,
        test_no_raw_episodes_passes_false,
        test_invalid_mode_rejected,
        test_max_chars_truncates_long_draft,
        test_format_draft_text_none,
        test_format_draft_text_short,
    ...
    print("=" * 60)
    print("Running test_v2_inspect_snapshot_draft tests")
    print("=" * 60)
